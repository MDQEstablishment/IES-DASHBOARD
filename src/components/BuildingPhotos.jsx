      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        {canUpload && (
          <>
            ...
            <button onClick={() => fileRef.current?.click()} disabled={busy} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', borderRadius: 'var(--radius-s)', background: 'var(--accent)', color: 'var(--surface-1)', fontSize: 12, fontWeight: 700 }}>
      ...
      {rows.length === 0 ? <Empty icon="camera">No site photos yet. Uploads here are compressed and tagged “General”; Daily Report photos arrive tagged by date + ESM.</Empty> : (
        Object.entries(groups).map(([date, ps]) => (
          <div key={date} style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 8 }}>{date === 'Undated' ? 'UNDATED' : fmtDate(date).toUpperCase()}</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(120px,1fr))', gap: 10 }}>
              {ps.map((p) => (
                <button key={p.id} onClick={() => setIndex(slideIndexOf(p.id))} style={{ position: 'relative', aspectRatio: '1', borderRadius: 'var(--radius-m)', overflow: 'hidden', border: '1px solid var(--line)', background: 'var(--track)' }}>
                  ...
                  <span style={{ position: 'absolute', top: 5, left: 5, fontFamily: 'var(--mono)', fontSize: 8.5, fontWeight: 700, padding: '2px 6px', borderRadius: 'var(--radius-s)', color: 'var(--surface-1)', background: p.source === 'daily_report' ? 'rgba(160,118,43,.9)' : 'rgba(16,26,36,.7)' }}>
