...
function numberedIcon(n, color = 'var(--accent)') {
  return L.divIcon({
    className: '',
    html: `<div style="width:26px;height:26px;border-radius:50% 50% 50% 0;background:${color};transform:rotate(-45deg);box-shadow:0 2px 6px rgba(16,26,36,.35);display:flex;align-items:center;justify-content:center;border:2px solid var(--surface-1)"><span style="transform:rotate(45deg);color:var(--surface-1);font:700 12px/1 'JetBrains Mono',monospace">${n}</span></div>`,
    iconSize: [26, 26],
    iconAnchor: [13, 26],
    popupAnchor: [0, -24],
  })
...
export default function BuildingsMap({ buildings = [] }) {
  ...
  if (!pts.length) {
    return (
      <div style={{ height: 360, borderRadius: 'var(--radius-m)', background: 'var(--bg)', border: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-3)', fontSize: 13 }}>
  ...
  return (
    <MapContainer center={center} zoom={pts.length > 1 ? 6 : 12} scrollWheelZoom={false}
      style={{ height: 360, width: '100%', borderRadius: 'var(--radius-m)', border: '1px solid var(--line)' }}>
          ...
          <Marker key={b.id} position={[Number(b.location_lat), Number(b.location_lng)]} icon={numberedIcon(i + 1, col)}>
            <Popup>
              <div style={{ fontFamily: 'var(--ui)', minWidth: 180 }}>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text-3)' }}>{b.code}</div>
                <div style={{ fontWeight: 700, fontSize: 13, margin: '2px 0 6px' }}>{b.name}</div>
                <div style={{ fontSize: 12, color: 'var(--line-ctrl)' }}>🏗 {b.contractor_name || b.contractor || '—'}</div>
                {b.contractor_phone && <div style={{ fontSize: 12, color: 'var(--line-ctrl)' }}>📞 {b.contractor_phone}</div>}
                <div style={{ marginTop: 6, display: 'inline-block', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 'var(--radius-s)', color: col, background: statusMeta(b.status_override || 'pending')[1] }}>
