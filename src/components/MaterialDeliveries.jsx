...
export default function MaterialDeliveries({ projectId, buildings = [] }) {
  ...
  const { rows: projRows } = useLiveQuery('projects',
    (q) => q.select('id,code,name,region,client,project_reference_no,beneficiary_entity,contractor_name,doc_rev').eq('id', projectId), [projectId])
  const [add, setAdd] = useState(null) // draft row or null
  const [mirOpen, setMirOpen] = useState(false) // project-level Generate MIR modal
    ...
    <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4, flexWrap: 'wrap', gap: 8 }}>
        ...
        {canWrite && <div style={{ display: 'flex', gap: 8 }}>
          ...
          <Btn icon="plus" style={{ padding: '7px 11px', fontSize: 12 }} onClick={startAdd}>Add delivery</Btn>
        </div>}
      </div>
