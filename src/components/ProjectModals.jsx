...
export function ProjectFormModal({ mode = 'add', project, onClose }) {
  ...
  return (
    <Modal open width={640} title={mode === 'edit' ? `Edit project · ${project.code}` : 'Add project'} onClose={onClose}
      ...
      <Row>
        <Field label="Project code"><input style={inputStyle} value={f.code} onChange={(e) => set('code', e.target.value)} placeholder="PROJECT-A" /></Field>
      ...
      <Field label="Project name"><input style={inputStyle} value={f.name} onChange={(e) => set('name', e.target.value)} placeholder="Project A" /></Field>
      <Row>
        <Field label="Client"><input style={inputStyle} value={f.client} onChange={(e) => set('client', e.target.value)} placeholder="Entity A" /></Field>
...
export function ProjectImportModal({ onClose }) {
  ...
  const downloadTemplate = () => {
    const ws = utils.aoa_to_sheet([TEMPLATE_COLS, ['PROJECT-A', 'Project A', 'Entity A', 'Asir', 'active', '2025-09-01', '64', 'Al-Faisal HVAC', '+966 50 000 0000', 'MOI-001', 'Police HQ — Abha', 'Abha']])
