...
export default function CocDetailDrawer({ coc, buildings, esmName, onClose }) {
  ...
  const revBlock = (c) => {
    ...
    return (
      <div key={c.id} style={{ border: '1px solid ' + (current ? 'var(--accent)' : 'var(--line)'), borderRadius: 'var(--radius-m)', padding: '10px 12px' }}>
  ...
  return (
    <Drawer open width={460} title={coc.code} subtitle={`${kindLabel(coc.esm_codes, esmName)} certificate · ${(coc.esm_codes || []).join(' + ')}`} onClose={onClose}>
      <div style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 6 }}>COVERS</div>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 6 }}>REVISIONS</div>
