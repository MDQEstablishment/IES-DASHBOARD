      ...
      <div className="ies-bdrail" style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16, alignItems: 'start' }}>
                      ...
                      return (
                        <tr key={s.id} style={{ borderTop: '1px solid var(--line)' }}>
                          ...
                          <td style={{ padding: '9px 8px', textAlign: 'right' }}>
                            {canInstall
                              ? <input defaultValue={planned} type="number" min="0" onBlur={(e) => Number(e.target.value) !== planned && bgUpdate('building_item_scope', s.id, { planned_qty: Math.max(0, parseInt(e.target.value, 10) || 0) }, { okMsg: 'Planned updated' })} style={{ width: 70, padding: '5px 7px', border: '1px solid var(--line)', borderRadius: 6, fontSize: 12, fontFamily: 'var(--mono)', textAlign: 'right' }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 14 }}>
            ...
            <input disabled placeholder="Add a comment… (coming soon)" style={{ ...inputStyle, marginTop: 10, fontSize: 12.5, background: '#FAFBFC', color: 'var(--text-3)' }} />
          </div>
        </div>
      </div>
    ...
    <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 16 }}>
      ...
      {adding && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          <input style={{ ...inputStyle, flex: 1 }} placeholder="Room name (e.g. Classroom 101)" value={name} onChange={(e) => setName(e.target.value)} />
          <input style={{ ...inputStyle, width: 90 }} placeholder="Floor" value={floor} onChange={(e) => setFloor(e.target.value)} />
  ...
  return (
    <Modal open width={560} title="Add today’s install" onClose={onClose}
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <div style={{ flex: 1 }}><Field label="Quantity installed"><input style={inputStyle} type="number" min="1" value={qty} onChange={(e) => setQty(e.target.value)} placeholder="e.g. 12" /></Field></div>
        <div style={{ flex: 1 }}><Field label="Install date"><input style={inputStyle} type="date" value={date} onChange={(e) => setDate(e.target.value)} /></Field></div>
      </div>
      <div style={{ display: 'flex', gap: 12 }}>
        ...
        <div style={{ flex: 1 }}><Field label="Workers"><input style={inputStyle} type="number" min="0" value={workers} onChange={(e) => setWorkers(e.target.value)} /></Field></div>
      </div>
      <Field label="Photos (compressed to ≤200 KB, tagged by ESM + date)"><input type="file" accept="image/*" multiple onChange={(e) => setFiles([...(e.target.files || [])])} style={{ fontSize: 13 }} /></Field>
    </Modal>
  )
}
