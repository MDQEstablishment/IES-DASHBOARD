...
| R6 | Client identity | `projects.client` + `beneficiary_entity` + `entity_name_ar` | **Corrected (owner, sprint 1):** `client` = the **paying party** (Tarshid on TARSHID work — the MIR/WIR generator's `\|\| 'Tarshid'` default proves the semantics), `beneficiary_entity` = the entity whose buildings are retrofitted, `entity_name_ar` its Arabic rendering. Distinct facts that merely coincide while every contract is a Tarshid contract — **not duplicates; never derive or delete one from the other** | **Keep all three columns** (all 🔒: `cocPdf.js:222`, `inspectionDocs.js:25`, `progressReport.js:284`, `savingSheetGen.js:28-29`, `search.js:136`). Fix the *capture*: `beneficiary_entity` blank ⇒ **write** `client` into it (the form placeholder already claims "Defaults to Client" and lies — `ProjectModals.jsx:71` writes null) | 🔒 | 3→1 typical, 3 only when they genuinely differ |
