...
begin
  select id into v_proj from public.projects where code = 'PROJECT-A';
  ...
  if not exists (select 1 from public.project_documents where project_id = v_proj and name = 'PROJECT-A-COC-001') then
    ...
    values (v_proj, v_esm3, v_b1, 'coc', 'PROJECT-A-COC-001', 'A','A','approved','Eng. Khalid Al-Mutairi', now()-interval '7 days', now()-interval '20 days', v_pm)
  ...
  if not exists (select 1 from public.project_documents where project_id = v_proj and name = 'PROJECT-A-COC-002') then
    ...
    values (v_proj, v_esm1, v_b1, 'coc', 'PROJECT-A-COC-002', 'A','A','submitted', now()-interval '6 days', v_pm)
    returning id into v_coc;
    insert into public.coc_buildings select v_coc, b.id from public.buildings b where b.project_id = v_proj and b.status_override <> 'archived';
  ...
  if not exists (select 1 from public.project_documents where project_id = v_proj and name = 'PROJECT-A-COC-003') then
    insert into public.project_documents (project_id, esm_id, building_id, doc_type, name, revision, version, status, client_reviewer_name, client_response_date, submitted_at, submitted_by)
    values (v_proj, v_esm3, v_b1, 'coc', 'PROJECT-A-COC-003', 'A','A','approved','Eng. Sara Al-Ghamdi', now()-interval '5 days', now()-interval '15 days', v_pm)
    returning id into v_coc;
