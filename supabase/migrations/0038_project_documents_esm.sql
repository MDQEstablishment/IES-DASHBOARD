...
-- 3) Tag the existing untagged PROJECT-A document to ESM1 so it shows --------
update public.project_documents d
   set esm_id = (select id from public.esms where code = 'ESM1')
  from public.projects p
 where d.project_id = p.id and p.code = 'PROJECT-A' and d.esm_id is null;

-- 4) Seed a few ESM-tagged project-level docs for PROJECT-A (proof) ----------
