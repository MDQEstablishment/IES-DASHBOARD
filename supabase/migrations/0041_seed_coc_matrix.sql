...
-- Realistic per-(building, ESM) COCs on PROJECT-A with client reviewer names and
...
update public.project_documents d
   set building_id = (select b.id from public.buildings b join public.projects p on p.id=b.project_id
                      where p.code='PROJECT-A' and b.code='MOI-001')
  from public.projects p
 where d.project_id = p.id and p.code='PROJECT-A' and d.name='COCs' and d.building_id is null;
...
join public.projects p on p.code='PROJECT-A'
